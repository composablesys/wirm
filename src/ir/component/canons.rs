...
impl Canons {
    pub fn new(items: Vec<CanonicalFunction>) -> Self {
        ...
        for i in items.iter() {
            if matches!(i, CanonicalFunction::Lift {..} | CanonicalFunction::Lower {..}) {
                num_lift_lower += 1;
            }
        }

        Self { items, num_lift_lower, ..Self::default() }
    }
    ...
    pub(crate) fn add(&mut self, canon: CanonicalFunction) -> (u32, CanonicalFuncId) {
        ...
        let fid_inner = match canon {
            ...
            _ => todo!("Haven't implemented support to add this canonical function type yet: {:#?}", canon)
        };
